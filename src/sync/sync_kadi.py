...
from src.config.settings import (
    DEVICE_TYPE,
    DEVICE_USER_KADI_ID,
    DEVICE_USER_PERSISTENT_ID,
    DEVICE_RECORD_PERSISTENT_ID,
...
class KadiSyncManager(ISyncManager):
    ...
    def _prepare_resources(
        ...
        db_user: KadiUser = self._get_db_user_from_local_record(
            db_manager, local_record
        )
        # Get or create the user rawdata group.
        user_group: KadiGroup = self._get_or_create_db_user_rawdata_group(
            db_manager, local_record, db_user
        )
        # Retrieve the device user.
        device_user: KadiUser = db_manager.user(
            username=DEVICE_USER_KADI_ID, identity_type="local"
        )
        # Get or create the device rawdata group.
        device_group: KadiGroup = self._get_or_create_db_device_rawdata_group(
            db_manager
        )
    ...
    def _get_or_create_db_user_rawdata_group(
        self, db_manager: KadiManager, local_record: LocalRecord, db_user: KadiUser
    ) -> KadiGroup:
        """
    ...
    def _get_or_create_db_device_rawdata_group(
        ...
        """
        db_device_record = db_manager.record(id=DEVICE_RECORD_PERSISTENT_ID)
        group_id = f"{DEVICE_USER_KADI_ID.lower()}{ID_SEP}rawdata{ID_SEP}group"
        title = f"{db_device_record.meta['title']}: Raw Data Records"
        add_user_info = {"user_id": DEVICE_USER_PERSISTENT_ID, "role": "admin"}
        return self._get_or_create_group(db_manager, group_id, title, add_user_info)
    ...
    def _get_or_create_db_record(
        ...
        db_record: KadiRecord = db_manager.record(
            create=True, identifier=local_record.identifier
        )
        return db_record
    ...
    def _get_db_user_from_local_record(
        ...
        try:
            db_user: KadiUser = db_manager.user(
                username=db_user_id, identity_type="local"
            )
        except Exception:
            self.ui.show_error(
    ...
    def _initialize_new_db_record(
        ...
        if not local_record.is_in_db:
            ...
            db_record.link_record(DEVICE_RECORD_PERSISTENT_ID, "generated by")
            db_record.add_group_role(group_id=db_user_group.id, role_name="member")
            db_record.add_group_role(
                group_id=db_device_data_group.id, role_name="member"
            )
    ...
    def sync_logs_to_database(self):
        ...
        """
        with self.db_manager as db_manager:
            db_record: KadiRecord = db_manager.record(id=DEVICE_RECORD_PERSISTENT_ID)
