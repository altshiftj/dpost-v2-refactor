...
class KadiSyncManager(ISyncManager):
    ...
    def sync_record_to_database(self, local_record: LocalRecord):
        ...
        try:
            with self.db_manager as db_manager:  # Step 1: Connect to DB
                ...
                filelist_response = db_record.get_filelist()  # Ensure the file list is up-to-date

                filelist_payload = filelist_response.json()

                filenames = [item['name'] for item in filelist_payload.get('items', [])]
                ...
                logger.info(f"All files for record '{local_record.identifier}' "
    ...
    def _initialize_new_db_record(
        ...
        if not local_record.is_in_db:
            ...
            db_record.add_tag('Electron Microscopy') #TODO: Make generic, defined in settings?

            # Add the datatype extracted from the local_record's identifier
