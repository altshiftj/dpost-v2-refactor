...
class SyncManager(ISyncManager):
    ...
    def sync_record_to_database(self, local_record: LocalRecord):
        ...
        """
        try:
            with self.db_manager_factory as db_manager:  # Step 1: Connect to DB
                ...
                db_group = self._create_or_retrieve_db_group(db_manager, local_record)

                db_user = self._create_or_retrieve_db_user(db_manager, local_record)
    ...
    def _create_or_retrieve_db_group(self, db_manager: KadiManager, local_record: LocalRecord) -> KadiGroup:
        """
        ...
        db_group_id = f"{user_initials}_{institute}_rawdata"
        db_group: KadiGroup = db_manager.group(create=True, identifier=db_group_id)
        return db_group
    
    def _create_or_retrieve_db_user(self, db_manager: KadiManager, local_record: LocalRecord) -> KadiUser:
        """
        ...
        """
        institute, user_initials, _ = local_record.short_id.split('_')
        db_user_id = f"{user_initials}_{institute}"
        try:
    ...
    def _initialize_new_db_record(self, db_record: KadiRecord, local_record: LocalRecord, db_user: KadiUser, db_group: KadiGroup):
        """
        ...
        if not local_record.is_in_db:
            ...
            db_record.add_user(user_id=DEVICE_RECORD_ID, role_name='admin')
            ...
            # Assign group role
            db_record.add_group_role(group_id=db_group.id, role_name='collaborator')
