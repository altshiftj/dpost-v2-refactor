"""Concrete V2 test PC plugin adapter exports."""

from __future__ import annotations

from types import MappingProxyType
from typing import Any, Mapping

from dpost_v2.application.contracts.context import RuntimeContext
from dpost_v2.application.contracts.plugin_contracts import (
    PLUGIN_CONTRACT_VERSION,
    PluginCapabilities,
    PluginMetadata,
)
from dpost_v2.plugins.pcs.test_pc.settings import (
    PcPluginSettings,
    validate_test_pc_settings,
)


def metadata() -> PluginMetadata:
    """Return stable metadata for concrete test PC plugin."""
    return PluginMetadata(
        plugin_id="test_pc",
        family="pc",
        version="1.0.0",
        contract_version=PLUGIN_CONTRACT_VERSION,
        supported_profiles=("prod",),
    )


def capabilities() -> PluginCapabilities:
    """Return capability flags for concrete test PC plugin."""
    return PluginCapabilities(
        can_process=False,
        supports_preprocess=False,
        supports_batch=False,
        supports_sync=True,
    )


def validate_settings(raw_settings: Mapping[str, Any]) -> PcPluginSettings:
    """Validate and normalize test PC settings payload."""
    return validate_test_pc_settings(raw_settings)


def create_sync_adapter(settings: Mapping[str, Any]) -> Mapping[str, Any]:
    """Create deterministic sync-adapter payload for test PC plugin."""
    normalized = validate_settings(settings)
    return MappingProxyType(
        {
            "endpoint": normalized.endpoint,
            "workspace_id": normalized.workspace_id,
            "upload_mode": normalized.upload_mode,
            "api_token": normalized.api_token,
        }
    )


def prepare_sync_payload(
    record: Mapping[str, Any],
    context: RuntimeContext,
) -> Mapping[str, Any]:
    """Prepare sync payload for test PC plugin."""
    _ = context
    record_id = record.get("record_id")
    if not isinstance(record_id, str) or not record_id.strip():
        raise ValueError("record_id is required")
    return MappingProxyType({"record_id": record_id.strip(), "plugin_id": "test_pc"})


def on_shutdown() -> None:
    """Run the optional lifecycle shutdown hook."""
