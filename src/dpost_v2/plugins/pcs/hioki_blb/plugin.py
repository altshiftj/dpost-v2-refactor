"""Concrete V2 hioki_blb PC plugin adapter exports."""
...
from dpost_v2.plugins.pcs.hioki_blb.settings import (
    PcPluginSettings,
    validate_hioki_blb_settings,
)


def metadata() -> PluginMetadata:
    """Return stable metadata for concrete hioki_blb plugin."""
...
def capabilities() -> PluginCapabilities:
    """Return capability flags for concrete hioki_blb plugin."""
...
def validate_settings(raw_settings: Mapping[str, Any]) -> PcPluginSettings:
    """Validate and normalize hioki_blb settings payload."""
    return validate_hioki_blb_settings(raw_settings)


def create_sync_adapter(settings: Mapping[str, Any]) -> Mapping[str, Any]:
    """Create deterministic sync-adapter payload for hioki_blb plugin."""
...
def prepare_sync_payload(
    ...
    """Prepare sync payload for hioki_blb plugin."""
    _ = context
    record_id = record.get("record_id")
    if not isinstance(record_id, str) or not record_id.strip():
        raise ValueError("record_id is required")
    return MappingProxyType({"record_id": record_id.strip(), "plugin_id": "hioki_blb"})
