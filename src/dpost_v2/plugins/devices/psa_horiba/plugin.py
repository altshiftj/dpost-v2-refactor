"""Concrete V2 psa_horiba device plugin adapter exports."""
...
from dpost_v2.plugins.devices.psa_horiba.processor import DeviceProcessor
from dpost_v2.plugins.devices.psa_horiba.settings import (
    DevicePluginSettings,
    validate_psa_horiba_settings,
)


def metadata() -> PluginMetadata:
    """Return stable metadata for concrete psa_horiba plugin."""
...
def validate_settings(raw_settings: Mapping[str, Any]) -> DevicePluginSettings:
    """Validate and normalize psa_horiba settings payload."""
    return validate_psa_horiba_settings(raw_settings)


def create_processor(settings: Mapping[str, Any]) -> DeviceProcessor:
    """Create the psa_horiba processor from normalized settings."""
