"""Concrete V2 rmx_eirich_el1 device plugin adapter exports."""
...
from dpost_v2.plugins.devices.rmx_eirich_el1.processor import DeviceProcessor
from dpost_v2.plugins.devices.rmx_eirich_el1.settings import (
    DevicePluginSettings,
    validate_rmx_eirich_el1_settings,
)


def metadata() -> PluginMetadata:
    """Return stable metadata for concrete rmx_eirich_el1 plugin."""
...
def capabilities() -> PluginCapabilities:
    """Return capability flags for concrete rmx_eirich_el1 plugin."""
...
def validate_settings(raw_settings: Mapping[str, Any]) -> DevicePluginSettings:
    """Validate and normalize rmx_eirich_el1 settings payload."""
    return validate_rmx_eirich_el1_settings(raw_settings)


def create_processor(settings: Mapping[str, Any]) -> DeviceProcessor:
    """Create the rmx_eirich_el1 processor from normalized settings."""
