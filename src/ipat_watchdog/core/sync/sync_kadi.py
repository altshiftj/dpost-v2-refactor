...
class KadiSyncManager(ISyncManager):
    ...
    def _initialize_new_db_record(
        ...
        if context.user_group:
            db_record.add_group_role(group_id=context.user_group.id, role_name="member")
        if context.device_group:
            db_record.add_group_role(group_id=context.device_group.id, role_name="member")
