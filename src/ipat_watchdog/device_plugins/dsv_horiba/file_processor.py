...
class FileProcessorDSVHoriba(FileProcessorABS):
    ...
    def device_specific_preprocessing(self, path: str) -> PreprocessingResult | None:
        ...
        if not bucket["ready"] and has_native and has_exported:
            bucket["ready"] = True
            logger.debug(
                "Dissolver batch ready for key '%s': %s", key, [str(f) for f in files]
            )
    ...
    def probe_file(self, filepath: str) -> FileProbeResult:
        ...
        return FileProbeResult.match(
            confidence=confidence, reason=f"Found dissolver markers (score={score})"
        )
    ...
    def device_specific_processing(
        ...
        if not bucket:
            destination = get_unique_filename(
                record_path, filename_prefix, src.suffix.lower()
            )
        ...
        raw_files = [
            candidate for candidate in files if candidate.suffix.lower() in raw_exts
        ]
        txt_files = [
            candidate for candidate in files if candidate.suffix.lower() == ".txt"
        ]
            ...
            try:
                ...
                for raw_file in raw_files:
                    try:
                        ...
                        logger.warning(
                            "Could not delete raw file '%s': %s", raw_file, exc
                        )
