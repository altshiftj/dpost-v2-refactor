...
class FileProcessorRHEKinexus(FileProcessorABS):
    ...
    def _handle_export(self, folder_key: str, state: _FolderState, path: Path) -> Optional[str]:
        ...
        if state.pending_raw:
            ...
            state.bucket.append(_Pair(export_path=path, raw_path=pending.path, created=time.time()))
            logger.debug("Kinexus: paired export %s with pending native %s", path, pending.path)
            return None

        sentinel = state.sentinel
        if sentinel and sentinel.export_path != path:
            logger.warning(
                "Kinexus: replacing pending sentinel %s with %s", sentinel.export_path, path
            )
        ...
        if sentinel is not None:
            ...
            for pair in batch_pairs:
                ...
                relocated_pairs.append(
                    _Pair(export_path=new_export, raw_path=new_raw, created=pair.created)
                )
    ...
    def device_specific_processing(
        ...
        for pair in batch.pairs:
            ...
            unique_export_path = Path(get_unique_filename(str(record_dir), base_prefix, pair.export_path.suffix.lower()))
            basename = unique_export_path.stem
            move_item(str(pair.export_path), str(unique_export_path))
            logger.debug("Kinexus: export moved %s -> %s", pair.export_path, unique_export_path)
            ...
            logger.debug("Kinexus: native zipped %s -> %s (arcname=%s)", pair.raw_path, zip_dest, arcname)
            processed += 1
        ...
        logger.debug("Kinexus: finalized %d pair(s) for prefix=%r", processed, base_prefix)
    ...
    def _next_sequence_basename(self, directory: Path, prefix: str) -> str:
        ...
        logger.debug("Kinexus: next sequence for prefix=%r in %s -> %s", prefix, directory, next_name)
        return next_name
        ...
        for folder_key in folder_keys:
            ...
            for pair in list(state.bucket):
                if now - pair.created > ttl:
                    for component, label in ((pair.export_path, "export"), (pair.raw_path, "native")):
                        if component.exists():
                            try:
                                ...
                                logger.exception(
                                    "Kinexus: failed moving stale %s %s", label, component
                                )
                else:
            ...
            if sentinel and now - sentinel.created > ttl:
                if sentinel.export_path.exists():
                    try:
                        ...
                        logger.exception(
                            "Kinexus: failed moving stale sentinel %s", sentinel.export_path
                        )
                state.sentinel = None
