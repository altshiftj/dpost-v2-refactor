...
from ipat_watchdog.core.processing.batch_models import CsvNgbPair as _Pair
from ipat_watchdog.core.processing.batch_models import FlushBatch as _FlushBatch
from ipat_watchdog.core.processing.batch_models import PendingPath as _PendingNGB
...
class FileProcessorPSAHoriba(FileProcessorABS):
    ...
    def probe_file(self, filepath: str) -> FileProbeResult:
        ...
        t = text.lower()
        score = sum(
            tok in t for tok in ["horiba", "partica", "la-960", "diameter"]
            ...
            for pair in list(state.bucket):
                if now - pair.created > ttl_seconds:
                    for pth, reason in (
                        ...
                        if pth.exists():
                            logger.info(
                                "PSA: moving '%s' to exception folder (%s)", pth, reason
                            )
                            try:
                                safe_move_to_exception(str(pth))
                            except Exception:
                                logger.exception(
                                    "PSA: failed to move %s to exception", pth
                                )
                    moved_pairs += 1
                else:
