"""Domain policies for reconstructing and aging staged batch artefacts."""
...
def reconstruct_pairs_from_stage(
    ...
    *,
    ...
    left_items = sorted(
        [path for path in stage_dir.iterdir() if left_filter(path)],
        key=lambda path: path.stat().st_mtime,
    )
    right_items = sorted(
        [path for path in stage_dir.iterdir() if right_filter(path)],
        key=lambda path: path.stat().st_mtime,
    )
    ...
    right_by_stem = {path.stem: path for path in right_items}
    ...
    remaining_right = [path for path in right_items if path not in used_right]
