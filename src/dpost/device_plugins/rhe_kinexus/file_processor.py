...
from dpost.domain.processing.batch_models import ExportRawPair as _Pair
from dpost.domain.processing.batch_models import FlushBatch as _FlushBatch
from dpost.domain.processing.batch_models import PendingPath as _PendingRaw
...
class FileProcessorRHEKinexus(FileProcessorABS):
    ...
    def probe_file(self, filepath: str) -> FileProbeResult:
        ...
        score = sum(1 for token in positives if token in text) - sum(
            1 for token in negatives if token in text
        )
        if score <= 0:
        ...
        return FileProbeResult.match(
            ...
            reason=f"Found Kinexus markers (score={score})",
        )
