...
class UnifiedRenameDialog(simpledialog.Dialog):
    def __init__(self, parent: tk.Tk, attempted_filename: str, violation_info: dict, title: Optional[str] = None):
        self.result = None
    ...
    def body(self, master: tk.Frame) -> tk.Widget:
        ...
        tk.Label(master, text="Invalid Filename Attempt:", font=("Arial", 10, "bold")).pack(pady=(5, 0))

        text_widget = tk.Text(master, height=2, width=len(self.attempted_filename)+6)
        ...
        for reason in self.violation_info.get("reasons", []):
            tk.Label(master, text=f"- {reason}", anchor="w", justify="left", wraplength=400).pack(fill="x", padx=10)

        # --- User Input Fields ---
        form_frame = tk.Frame(master)
        form_frame.pack(pady=(10, 0), padx=10, fill="x")

        tk.Label(form_frame, text="Name (Initials):").grid(row=0, column=0, sticky='e', padx=5, pady=2)
        tk.Label(form_frame, text="Institute (Initials):").grid(row=1, column=0, sticky='e', padx=5, pady=2)
        tk.Label(form_frame, text="Sample Name:").grid(row=2, column=0, sticky='e', padx=5, pady=2)
