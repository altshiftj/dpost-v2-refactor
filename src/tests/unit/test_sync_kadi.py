...
def test_get_or_create_db_user_group_existing(dummy_ui_instance, local_record):
    ...
    group = sync_mgr._get_or_create_db_user_rawdata_group(
        dummy_manager, local_record, user
    )
...
def test_get_or_create_db_record(dummy_ui_instance, local_record):
    ...
    dummy_manager.record.assert_any_call(
...
def test_initialize_new_db_record(dummy_ui_instance, local_record):
    ...
    dummy_record.link_record.assert_called_with(DEVICE_RECORD_PERSISTENT_ID, "generated by")
...
def test_upload_record_files_success(dummy_ui_instance):
    ...
    dummy_record.upload_file = MagicMock(
        side_effect=lambda path, force=False: dummy_record.uploaded_files.append(
            (path, force)
        )
    )
    sync_mgr = KadiSyncManager(ui=dummy_ui_instance)
