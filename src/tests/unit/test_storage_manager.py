...
def test_move_item_fallback_shutil_move(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(Path, "rename", lambda self, dest: (_ for _ in ()).throw(OSError("Simulated rename failure")))
...
def test_move_to_exception_folder(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(PathManager, "get_exception_path", lambda name: fake_unique_path(name))
...
def test_move_to_rename_folder(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(PathManager, "get_rename_path", lambda name, base_dir=None: fake_unique_path(name, base_dir))
...
def test_move_to_record_folder(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(PathManager, "get_record_path", lambda filename_prefix: str(dest_file))
