...
def record_manager(fake_sync, tmp_settings):
    with patch("records.record_manager.load_persisted_records", return_value={}):
...
def test_create_record_generates_proper_id_and_sample(record_manager):
    with patch("records.record_manager.generate_record_id", return_value="dev-usr-ipat-sample_a"):
...
def test_add_item_to_record_saves_it(tmp_path, record_manager):
    ...
    with patch("records.record_manager.save_persisted_records") as mock_save:
...
def test_sync_records_to_database_uploads_ipat(record_manager):
    ...
    with patch("records.record_manager.save_persisted_records"):
...
def test_persist_records_dict_lazy_loads_once(fake_sync, tmp_settings):
    with patch("records.record_manager.load_persisted_records",
