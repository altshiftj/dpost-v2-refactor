...
def test_create_record_generates_proper_id_and_sample(record_manager):
    with patch('src.records.id_generator.IdGenerator.generate_record_id', return_value="dev-usr-ipat-sample_A"):
...
def test_persist_records_dict_lazy_loads_once(mock_sync_manager):
    with patch('src.records.record_persistence.load_persisted_records', return_value={"x": LocalRecord(identifier="x")}) as mock_load:
