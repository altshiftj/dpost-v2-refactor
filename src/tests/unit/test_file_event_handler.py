...
def handler(event_queue, tmp_settings):
    ...
    with patch("handlers.file_event_handler.SettingsStore.get", return_value=tmp_settings):
...
def test_on_created_tiff(handler, event_queue, tmp_path):
    ...
    with patch("handlers.file_event_handler.Timer") as mock_timer:
...
def test_path_no_longer_exists(handler, event_queue, tmp_path):
    ...
    with patch("pathlib.Path.exists", return_value=False), patch(
        "handlers.file_event_handler.Timer"
    ) as mock_timer, patch("handlers.file_event_handler.logger.warning") as mock_warn:
        handler.on_created(event)
