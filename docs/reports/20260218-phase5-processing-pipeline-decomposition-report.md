...
2. Route decision and non-ACCEPT handling now have explicit seams and rename retries no longer recurse through `_route_with_prefix()`, but rename prompt loops and unappendable warning side effects still live in one orchestration class (`src/ipat_watchdog/core/processing/file_process_manager.py`).
