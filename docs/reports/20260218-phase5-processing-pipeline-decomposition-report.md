...
| Persist/sync trigger | `_persist_and_sync_stage()` + `FileProcessManager._persist_candidate_record_stage()` + `FileProcessManager.add_item_to_record()` | ACCEPT delegation in `_persist_and_sync_stage()`; manager seam in `_persist_candidate_record_stage()`; record persistence updates and immediate sync trigger in `add_item_to_record()` | Persist stage seam is explicit and now delegates through an explicit manager persistence seam, but underlying record mutation/metrics/sync side effects remain concentrated in `add_item_to_record()`. |
...
3. `add_item_to_record()` still combines plugin processing, record mutation, force-path expansion, metrics, and immediate sync in one hotspot (`src/ipat_watchdog/core/processing/file_process_manager.py`), even though ACCEPT pipeline flow now delegates through `_persist_candidate_record_stage()`.
