# ADR-0002: Framework-first Sequencing for dpost Delivery
...
## Context
- Runtime modernization work can drift if concrete integrations (device plugins, sync backends) are moved before stable framework contracts exist.
...
## Decision
- Apply framework-first sequencing for runtime delivery:
...
- Parallel framework and concrete delivery:
...
- requires discipline to keep concrete integrations out of kernel-first phases
- Neutral:
- existing concrete integrations can still be validated through legacy paths until retired

## Implementation Notes
- Update phase definitions/checklists to include framework-first gates.
- Require architecture boundary tests for kernel contracts before concrete adapter/plugin adoption.
