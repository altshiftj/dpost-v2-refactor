...
| `dpost.infrastructure.runtime.bootstrap_dependencies` | Infrastructure adapter | Own runtime bootstrap dependency construction for config, UI adapters, sync manager, and app object wiring | Runtime dependency factories and legacy-backed runtime construction seams | Startup settings policy and composition-root selection logic |
| `dpost.infrastructure.runtime.config_dependencies` | Infrastructure adapter | Encapsulate runtime config dependency wiring for bootstrap construction during staged config retirement | Config-service and runtime directory initialization dependency bindings | Runtime composition policy and startup settings decisions |
| `dpost.infrastructure.runtime.desktop_ui` | Infrastructure adapter | Resolve desktop UI class behind a dpost-owned runtime boundary | Desktop UI class import boundary and lazy loading seam | Runtime mode decision policy |
...
| `dpost.plugins.contracts` | Plugin contract | Define dpost-owned structural plugin protocol types for loader boundaries | Runtime-checkable plugin type contracts and decoupled typing surface | Concrete plugin discovery implementation |
...
| `dpost.application.runtime.runtime_dependencies` | Application boundary helper | Encapsulate legacy-backed runtime app imports while rehost is incomplete | Runtime app dependency bindings for config/processing/session/metrics | Runtime orchestration logic and decision policy |
...
| `dpost.infrastructure.storage.filesystem_utils` | Infrastructure adapter | Provide canonical storage/path/filename utility behavior for dpost processing and records flows | Directory init, move helpers, naming/ID helpers, record persistence serialization | Runtime composition policy and device-processing orchestration |
