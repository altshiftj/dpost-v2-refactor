...
| `dpost.application.naming.policy` | Application support | Apply active runtime naming configuration to domain naming policy helpers | Config-aware naming facade for routing, rename flow, and plugins | Filesystem side effects and low-level path mutation |
...
| `dpost.infrastructure.storage.filesystem_utils` | Infrastructure adapter | Provide canonical storage/path/filename utility behavior for dpost processing and records flows | Directory init, path/move helpers, record ID helpers, record persistence serialization | Runtime composition policy, domain naming validation policy, and device-processing orchestration |
