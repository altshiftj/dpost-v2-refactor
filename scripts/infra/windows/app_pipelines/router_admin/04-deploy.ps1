...
$buildVersionPath = "build\version.txt"
$deployVersionPath = "version.txt"
...
# Produce deploy version.txt (augment build/version.txt with deploy info)
$deployMeta = @"
...
$items = @(
  ...
  @{ Local = $deployVersionPath;  Label = 'version.txt'; Remote = 'version.txt' }
)
foreach ($it in $items) {
  $localPath      = $it.Local
...
} else { Write-Warning "MISSING: Version file not found" }
