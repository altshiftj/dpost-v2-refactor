...
class FileProcessor:
    ...
    def _construct_names_and_id(self, base_name, extension):
        ...
        if len(parts) == 3:
            ...
            base_name = f"{institute}_{user_ID}_{sample_ID}_{date}"
        elif len(parts) == 4:
            institute, user_ID, sample_ID, date = parts
        else:
            raise ValueError("Invalid base name format.")
        ...
        unique_new_path = self._get_unique_path(new_path)
    ...
    def _attempt_rename(self, path, base_name, extension, is_folder, notify=True):
        try:
            scrubbed_name, record_ID, new_path = self._construct_names_and_id(base_name, extension)
            self._move_item(path, new_path, is_folder)

            if is_folder and self.data_type == 'elid':
                self._rename_elid_files(new_path, scrubbed_name)
            ...
            if self._record_in_archive(scrubbed_name):
                if self.gui_manager.prompt_append_record(scrubbed_name):
                    self._update_and_manage_session(scrubbed_name, record_ID, new_path)
                else:
    ...
    def _update_and_manage_session(self, base_name, record_ID, new_path):
        self._update_record(base_name, record_ID, new_path)
