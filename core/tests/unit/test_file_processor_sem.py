...
def test_device_specific_processing_tif_branch(processor, tmp_path):
    ...
    with patch(
        "src.processing.file_processor_sem.PathManager.get_unique_filename",
        return_value=unique_filename,
    ) as mock_get_unique:
        with patch(
            "src.processing.file_processor_sem.StorageManager.move_item"
        ) as mock_move:
            result, datatype = processor.device_specific_processing(
                src_file, str(record_dir), filename_prefix, extension
            )
            assert result == unique_filename
            assert datatype == "img"
            mock_move.assert_called_once_with(src_file, unique_filename)
            mock_get_unique.assert_called_once_with(
                str(record_dir), filename_prefix, extension
            )
