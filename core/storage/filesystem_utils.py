...
from core.ui.ui_messages import ValidationMessages
...
def get_record_path(filename_prefix: str) -> str:
    settings: BaseSettings = SettingsStore.get()
    user_ID, institute, sample_ID = filename_prefix.split(settings.ID_SEP)
    record_path = Path(settings.DEST_DIR) / institute.upper() / user_ID.upper() / sample_ID
...
def get_unique_filename(directory: str, filename_prefix: str, extension: str) -> str:
    ...
    for existing in dir_path.iterdir():
        if existing.is_file() and existing.suffix == extension:
            ...
            prefix_no_counter = existing_prefix.rsplit(settings.ID_SEP, 1)[0]
            if prefix_no_counter in filename_prefix:
                try:
                    suffix_num = int(existing_prefix.rsplit(settings.ID_SEP, 1)[1])
                    if suffix_num >= counter:
                        counter = suffix_num + 1
                except (ValueError, IndexError):
                    continue
    while True:
        candidate_name = f"{filename_prefix}{settings.ID_SEP}{counter:02d}{extension}"
...
def get_rename_path(name: str, base_dir: Optional[str] = None) -> str:
    settings: BaseSettings = SettingsStore.get()
    base_dir = base_dir or settings.RENAME_DIR
...
def get_exception_path(name: str, base_dir: Optional[str] = None) -> str:
    settings: BaseSettings = SettingsStore.get()
    base_dir = base_dir or settings.EXCEPTIONS_DIR
    filename_prefix, extension = Path(name).stem, Path(name).suffix
    return get_unique_filename(base_dir, filename_prefix, extension)
...
def generate_record_id(filename_prefix: str) -> str:
    settings: BaseSettings = SettingsStore.get()
    return f"{settings.DEVICE_TYPE}{settings.ID_SEP}{filename_prefix}".lower()

def generate_file_id(filename_prefix: str) -> str:
    settings: BaseSettings = SettingsStore.get()
    user_id, institute, sample_id = filename_prefix.split(settings.ID_SEP)
    return f"{settings.DEVICE_TYPE}{settings.ID_SEP}{sample_id}"
...
def load_persisted_records() -> dict[str, LocalRecord]:
    settings: BaseSettings = SettingsStore.get()
    json_path = Path(settings.DAILY_RECORDS_JSON)
...
def is_valid_prefix(raw_prefix: str) -> bool:
    settings: BaseSettings = SettingsStore.get()
    if not settings.FILENAME_PATTERN.match(raw_prefix):
        logger.debug(f"Prefix '{raw_prefix}' failed regex match.")
        return False
    return raw_prefix.count(settings.ID_SEP) >= 2

def sanitize_prefix(raw_prefix: str) -> str:
    settings: BaseSettings = SettingsStore.get()
    parts = raw_prefix.strip().split(settings.ID_SEP)
    if len(parts) < 3:
        return raw_prefix
    user_id = parts[0].strip()
    institute = parts[1].strip()
    sample_id = settings.ID_SEP.join(part.strip() for part in parts[2:]).replace(" ", "_")
    return f"{user_id.lower()}{settings.ID_SEP}{institute.lower()}{settings.ID_SEP}{sample_id}"
...
def explain_filename_violation(filename: str) -> dict:
    ...
    if not settings.FILENAME_PATTERN.match(filename):
        result["valid"] = False
        segments = filename.split(settings.ID_SEP)

        if len(segments) != 3:
            result["reasons"].append(ValidationMessages.MISSING_SEPARATOR)
            for i, char in enumerate(filename):
                if char == settings.ID_SEP:
...
def analyze_user_input(dialog_result: dict | None) -> dict:
    ...
    raw_prefix = f"{user_id}{settings.ID_SEP}{institute}{settings.ID_SEP}{sample_id}"
