...
from core.ui.ui_abstract import UserInterface
from core.ui.ui_messages import ErrorMessages
from core.records.local_record import LocalRecord
from core.sync.sync_abstract import ISyncManager
...
class KadiSyncManager(ISyncManager):
    ...
    def _prepare_resources(self, db_manager: KadiManager, local_record: LocalRecord) -> SyncResources:
        ...
        device_user = db_manager.user(username=self.settings.DEVICE_USER_KADI_ID, identity_type="local")
    ...
    def _get_or_create_db_device_rawdata_group(self, db_manager: KadiManager) -> KadiGroup:
        sep = self.settings.ID_SEP
        device_record_id = self.settings.DEVICE_RECORD_PERSISTENT_ID
        db_device_record = db_manager.record(id=device_record_id)
        group_id = f"{self.settings.DEVICE_USER_KADI_ID.lower()}{sep}rawdata{sep}group"
        title = f"{db_device_record.meta['title']}: Raw Data Records"
        add_user_info = {"user_id": self.settings.DEVICE_USER_PERSISTENT_ID, "role": "admin"}
        return self._get_or_create_group(db_manager, group_id, title, add_user_info)
    ...
    def _get_db_user_from_local_record(self, db_manager: KadiManager, local_record: LocalRecord) -> Optional[KadiUser]:
        user_id = f"{local_record.user}{self.settings.ID_SEP}{local_record.institute}"
        try:
    ...
    def _initialize_new_db_record(
        ...
        if not local_record.is_in_db:
            ...
            db_record.set_attribute("description", self.settings.DEFAULT_RECORD_DESCRIPTION)
            db_record.set_attribute("type", "rawdata")
            for tag in self.settings.RECORD_TAGS:
                db_record.add_tag(tag)
            db_record.add_tag(local_record.datatype)
            db_record.link_record(self.settings.DEVICE_RECORD_PERSISTENT_ID, "generated by")
    ...
    def sync_logs_to_database(self):
        with self.db_manager as db_manager:
            db_record = db_manager.record(id=self.settings.DEVICE_RECORD_PERSISTENT_ID)
            db_record.upload_file(self.settings.LOG_FILE, force=True)
