...
def test_get_db_user_from_local_record_user_found(sync_mgr, local_record):
    ...
    user = sync_mgr._get_db_user_from_local_record(dummy_mgr, local_record, settings)
...
def test_get_or_create_db_user_group_existing(sync_mgr, local_record):
    ...
    settings = sync_mgr.settings_manager.get_composite_settings()
    group = sync_mgr._get_or_create_db_user_rawdata_group(dummy_mgr, local_record, user, settings)
...
def test_initialize_new_db_record(sync_mgr, local_record):
    ...
    dummy_record.link_record.assert_called_with(
        settings.DEVICE_RECORD_PERSISTENT_ID, "generated by"
    )
