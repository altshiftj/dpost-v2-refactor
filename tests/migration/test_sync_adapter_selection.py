...
def _install_missing_kadi_module(monkeypatch: pytest.MonkeyPatch) -> None:
    """Install a module stub that emulates a missing `kadi_apy` dependency."""

    class MissingKadiModule(ModuleType):
        """Raise ModuleNotFoundError when the adapter class is requested."""

        def __getattr__(self, name: str) -> object:
            if name == "KadiSyncAdapter":
                raise ModuleNotFoundError("No module named 'kadi_apy'")
            raise AttributeError(name)

    monkeypatch.setitem(
        sys.modules,
        "dpost.infrastructure.sync.kadi",
        MissingKadiModule("dpost.infrastructure.sync.kadi"),
    )
...
def test_compose_bootstrap_default_noop_when_kadi_dependency_missing(
    monkeypatch,
) -> None:
    ...
    _install_missing_kadi_module(monkeypatch)
    monkeypatch.setattr(bootstrap_module, "bootstrap", fake_bootstrap)
...
def test_compose_bootstrap_kadi_without_dependency_raises_clear_startup_error(
    monkeypatch,
) -> None:
    ...
    with pytest.raises(StartupError, match="optional dependency 'kadi_apy'"):
        composition.compose_bootstrap()
