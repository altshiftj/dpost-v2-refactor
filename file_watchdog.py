...
class FileMonitorApp:
    ...
    def handle_exception(self, exc_type, exc_value, exc_traceback):
        ...
        messagebox.showerror(
            "Application Error",
            "The filename monitor has encountered an unexpected error. Please contact the administrator.",
    ...
    def check_file_name(self, file_path):
        ...
        if pattern.match(base_name):
            # The file name matches the pattern; proceed to rename with counter
    ...
    def rename_file(self, file_path, new_name):
        """
        Renames the file to the new name provided by the user, appending a counter.
        """
        base_name, extension = os.path.splitext(new_name)
        counter = self.get_next_counter(base_name, extension)
        new_filename = f"{base_name}_{counter}{extension}"
        new_path = os.path.join(self.processed_dir, new_filename)

        try:
            os.rename(file_path, new_path)
            messagebox.showinfo("Success", f"File renamed to '{new_filename}'", parent=self.dialog_parent)
    ...
    def get_next_counter(self, base_name, extension):
        """
        Gets the next counter value for files with the given base name and extension.
        """
        existing_files = os.listdir(self.processed_dir)
        counters = []
        pattern = re.compile(rf'^{re.escape(base_name)}_(\d+){re.escape(extension)}$', re.IGNORECASE)
